...
use FindBin qw($Bin);
use lib "$Bin";
use lib qw(/home/rrichter/tmp/grid/phmmsearch/lib);
use TIGR::HTC::HMM;
...
&usage if ($options{help} || $options{usage});
...
if (defined($options{conf})) {
    ...
    $options{group}            ||= $cfg->val($section, "group");
...
push (@errors, "Username must be specified.")
    unless (defined($options{username}));
...
sub usage {
    ...
    -g --group=<group>             What group should this job be
                                   associated with.
